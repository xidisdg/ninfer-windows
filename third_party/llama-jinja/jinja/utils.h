...
static void string_replace_all(std::string& s, const std::string& search,
...
static std::string peak_source(const std::string& source, size_t pos, size_t max_peak_chars = 40) {
    if (source.empty()) { return "(no source available)"; }
    std::string output;
...
struct hasher {
    ...
    static constexpr auto block_size =
        sizeof(size_t); // in bytes; allowing the compiler to vectorize the computation
    ...
    //   - update(a ~ b) == update(a).update(b) with ~ as concatenation operator --> useful for
    //   streaming
    ...
    hasher& update(const std::string& s) noexcept { return update(s.data(), s.size()); }
    ...
    size_t digest() noexcept {
        ...
        if (idx > 0) {
            for (size_t i = idx; i < block_size; ++i) { buffer[i] = 0; }
            update_block(buffer);
            idx = 0;
        }
...
private:
    ...
    void update_block(const uint8_t* block) noexcept {
        size_t blk = static_cast<uint32_t>(block[0]) | (static_cast<uint32_t>(block[1]) << 8) |
        ...
        if constexpr (block_size == 8) {
            blk = blk | (static_cast<uint64_t>(block[4]) << 32) |
                  ...
                  (static_cast<uint64_t>(block[6]) << 48) | (static_cast<uint64_t>(block[7]) << 56);
        }
        state ^= blk;
        state *= prime;
    }
